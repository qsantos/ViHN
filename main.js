chrome.storage.sync.get((options) => {
    ...
    chrome.storage.sync.onChanged.addListener((changes) => {
        ...
        if (newestItemsOption === true) {
            ...
            focusNewest = false;
            if (newestItemsContainer) {
                newestItemsContainer.style.display = "none";
            }
        }
    });
    ...
    function initNewestItems() {
        if (newestItemsContainer !== undefined) {
            newestItemsContainer.style.display = "block";
            return;
        }
        ...
        newestItemsContainer.innerHTML = parts.join("");
    ...
    function newestEvent(event) {
        if (event.key === "n") {
            ...
            /* Jump 10 down */
            gotoNewestIndex(
                Math.min(currentNewestIndex + 10, newestItems.length - 1),
            );
        } else if (event.key === "K") {
            /* Jump 10 up */
            ...
            /* Last newest */
            gotoNewestIndex(newestItems.length - 1);
        } else {
            return;
        }
        event.stopPropagation();
